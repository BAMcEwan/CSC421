...
public class Search {
	...
	public String IterativeDeepeningTreeSearch() {
		...
		String solution = null;
	 	int i = 1;
		while(solution == null) {
			solution = TreeSearchDepthLimited(frontier, i);
			i++;
		}
		return solution;
	}
	...
	public String IterativeDeepeningGraphSearch() {
		...
		int i = 1;
		while(solution == null) {
			solution = GraphSearchDepthLimited(frontier, i);
			i++;
		}
		return solution;
	}
	...
	private void PrintTree(Node node) {
		...
		String order = node.order != -1 ? "order=" + node.order : "";
		System.out.print(node.state + "(g=" + node.path_cost + ", h=" + problem.h(node.state) + ", f=" + (node.path_cost + problem.h(node.state)) + ") " + order + "\n");
